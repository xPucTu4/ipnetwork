...
namespace System.Net.ConsoleApplication
{
    ...
    public class Program
    {
        ...
        private static readonly ArgParsed[] ArgsList = new[]
        {
            ...
            {
                if (!IPNetwork3.IPNetwork.TryParseCidr(arg, Sockets.AddressFamily.InterNetwork, out byte? cidr))
                {
            ...
            {
                if (!Program.TryParseIPNetwork(arg, ac.CidrParse, ac.CidrParsed, out IPNetwork3.IPNetwork ipnetwork))
                {
        ...
        {
            foreach (IPNetwork3.IPNetwork ipnetwork in ac.Networks)
            {
        ...
        {
            if (!IPNetwork3.IPNetwork.TryWideSubnet(ac.Networks, out IPNetwork3.IPNetwork widesubnet))
            {
        ...
        {
            if (!IPNetwork3.IPNetwork.TrySupernet(ac.Networks, out IPNetwork3.IPNetwork[] supernet))
            {
        ...
        private static void PrintNetworks(ProgramContext ac, IEnumerable<IPNetwork3.IPNetwork> ipnetworks, BigInteger networkLength)
        {
            int i = 0;
            foreach (IPNetwork3.IPNetwork ipn in ipnetworks)
            {
                i++;
        ...
        private static void SubnetNetworks(ProgramContext ac)
        {
            ...
            foreach (IPNetwork3.IPNetwork ipnetwork in ac.Networks)
            {
                ...
                if (!ipnetwork.TrySubnet(ac.SubnetCidr, out IPNetwork3.IPNetworkCollection ipnetworks))
                {
        ...
        private static void PrintNetwork(ProgramContext ac, IPNetwork3.IPNetwork ipn)
        {
        ...
        {
            var ipnetworks = new List<IPNetwork3.IPNetwork>();
            foreach (string ips in ac.NetworksString)
            {
                if (!Program.TryParseIPNetwork(ips, ac.CidrParse, ac.CidrParsed, out IPNetwork3.IPNetwork ipnetwork))
                {
        ...
        private static bool TryParseIPNetwork(string ip, CidrParseEnum cidrParseEnum, byte cidr, out IPNetwork3.IPNetwork ipn)
        {
            IPNetwork3.IPNetwork ipnetwork = null;
            if (cidrParseEnum == CidrParseEnum.Default)
            {
                if (!IPNetwork3.IPNetwork.TryParse(ip, out ipnetwork))
                {
                    ipn = null;
                    return false;
                }
            ...
            {
                if (!IPNetwork3.IPNetwork.TryParse(ip, cidr, out ipnetwork))
                {
                    if (!IPNetwork3.IPNetwork.TryParse(ip, out ipnetwork))
                    {
                        ipn = null;
